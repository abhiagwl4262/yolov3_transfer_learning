...
class Bit_yolov3(torch.nn.Module):
    def __init__(self, darknet_model, Bit_model):
        ...
        #self.mapping_layer1 = darknet_model.module_list[81]   # 1024 to 18
        #self.mapping_layer2 = darknet_model.module_list[93]   # 512 to 18
        #self.mapping_layer3 = darknet_model.module_list[105] # 512 to 18

        self.mapping_layer1 = nn.Sequential(nn.BatchNorm2d(1024), 
                                            nn.ReLU(), 
                                            nn.Conv2d(in_channels=1024, out_channels=18, kernel_size=(1,1)))   # 512 to 18
        self.mapping_layer2 = nn.Sequential(nn.BatchNorm2d(2048), 
                                            nn.ReLU(),
                                            nn.Conv2d(in_channels=2048, out_channels=18, kernel_size=(1,1))) # 1024 to 18
        self.mapping_layer3 = nn.Sequential(nn.BatchNorm2d(4096), 
                                            nn.ReLU(),
                                            nn.Conv2d(in_channels=4096, out_channels=18, kernel_size=(1,1))) # 2048 to 18
        self.seen = 0
    ...
    def forward(self, x, targets=None):
        ...
        out1, layer_loss = self.yolo_layers[2](x2, targets, img_dim)
        loss += layer_loss
        ...
        out3, layer_loss = self.yolo_layers[0](x4, targets, img_dim)
        loss += layer_loss
...
if __name__ == "__main__":
    ...
    for epoch in range(opt.epochs):
        ...
        for batch_i, (_, imgs, targets) in enumerate(dataloader):
            ...
            loss.backward()
            losses.update(loss.item(), imgs.size())
